...
Package.on_use((api, where) => {
  ...
  api.use('useraccounts:flow-routing@1.14.2', ['client', 'server'], {
    weak: true,
  });
  api.use('useraccounts:iron-routing@1.14.2', ['client', 'server'], {
    weak: true,
  });
...
Package.on_test((api) => {
  api.use(['c316:bootstrap4', 'useraccounts:core@1.14.2']);

  api.use(
    ['accounts-password', 'tinytest', 'test-helpers'],
    ['client', 'server']
  );
